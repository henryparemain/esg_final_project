...
page = st.sidebar.radio("Go to", ["2021 ESG Insights", "Custom ESG Insights"])
...
if page == "2021 ESG Insights":
    st.title("2021 ESG Insights")
...
# -----PAGE 2------
...
elif page == "Custom ESG Insights":
    st.title("Custom ESG Insights")
    ...
    if uploaded_file is not None:
        ...
        #drop down menu to allow user to select the number of paragraphs that they want to see
        selected_number = st.selectbox('Select the number of paragraphs', numbers)
        #drop down menu to allow user to select the type of paragraphs that they want to see
        selected_cat = st.selectbox('Select a category', categories)
        #selecting paraphs with more than 10 clean tokens 
        greater_than_10 = df_probs[df_probs['clean_paras'].apply(lambda x: len(x))>10]

        if selected_cat == 'Environmental':
            top3e = greater_than_10.sort_values('E',ascending=False)
            st.subheader('Most Important Environmental Related Paragraph:')
            for i in list(range(0,selected_number)):
                st.write(top3e.iloc[i,0])

        if selected_cat == 'Social':
            top3s = greater_than_10.sort_values('S',ascending=False)
            st.subheader('Most Important Social Related Paragraph:')
            for i in list(range(0,selected_number)):
                st.write(top3s.iloc[i,0])

        if selected_cat == 'Governance':
            top3g = greater_than_10.sort_values('G',ascending=False)
            st.subheader('Most Important Governance Related Paragraph:')
            for i in list(range(0,selected_number)):
                st.write(top3g.iloc[i,0])
        ...
        # background-color: #F08080;color:white;font-size:20px;height:3em;width:30em;border-radius:10px 10px 10px 10px;
        # }
        # """, unsafe_allow_html=True)

        if st.button('Click to explore topics in more depth'):
